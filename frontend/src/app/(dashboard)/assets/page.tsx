      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <div className="flex items-center gap-2 text-xs mb-2" style={{ color: "var(--text-muted)" }}>
            ...
            <span style={{ color: "var(--text-secondary)" }}>{t("assets")}</span>
          </div>
          <h1 className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>{t("assets")}</h1>
          <p className="text-sm mt-0.5" style={{ color: "var(--text-muted)" }}>
            {assets.length} {assets.length === 1 ? "asset" : "assets"} terdaftar
          </p>
        </div>
              ...
              <tbody>
                {assets.map(asset => {
                  ...
                  return (
                    <tr key={asset.id}>
                      <td>
                        <span className="font-semibold" style={{ color: "var(--text-primary)" }}>{asset.assetName}</span>
                      </td>
                      <td>
                      ...
                      <td className="text-right font-mono text-sm font-semibold" style={{ color: "var(--text-primary)" }}>
                        {fmt(Number(asset.purchasePrice))}
                      </td>
                      <td className="text-right font-mono text-sm" style={{ color: Number(asset.totalMaintenanceCost) > 0 ? "#f59e0b" : "var(--text-muted)" }}>
                        {fmt(Number(asset.totalMaintenanceCost))}
                      </td>
                      ...
                      <td>
                        <div className="flex items-center justify-center gap-1">
                          <button onClick={() => handleEdit(asset)} className="w-8 h-8 rounded-lg flex items-center justify-center transition-all hover:bg-violet-500/10"
                            style={{ color: "var(--text-muted)" }} title={t("edit")}>
                            <Pencil className="w-3.5 h-3.5" />
                          </button>
                          <button onClick={() => handleDelete(asset.id)} className="w-8 h-8 rounded-lg flex items-center justify-center transition-all hover:bg-red-500/10 hover:text-red-400"
                            style={{ color: "var(--text-muted)" }} title={t("delete")}>
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-base font-bold" style={{ color: "var(--text-primary)" }}>
                  {editingId ? t("edit") : t("create")} Asset
                </h2>
                <p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>
                  {editingId ? "Update detail aset" : "Tambah aset baru ke portofolio"}
                </p>
              </div>
              <button onClick={() => setShowForm(false)} className="w-8 h-8 rounded-lg flex items-center justify-center btn-ghost p-0">
                <X className="w-4 h-4" />
              </button>
            </div>
